...
// get hash from uint by XOR and product
uint uhash11(uint n) {
    n ^= (n << u.x);
    n ^= (n >> u.x);
    n *= k.x;
...
float hash11(float p) {
    ...
    return float(uhash11(n)) / float(UINT_MAX);  // normalize
}
...
float vnoise21(vec2 p) {
    ...
    vec2 f = fract(p);
    f = f * f * f * (10.0 - 15.0 * f + 6.0 * f * f);  // Hermite interpolation
...
void main(){
    ...
    if (channel[0] == 0){  // left ... value noise
        if (channel[1] == 0){  // left bottom
            v = vnoise21(pos);
        } else {  // left top
            v = vnoise31(vec3(pos, u_time));
        }
    } else{  // right ... gradient noise
        if (channel[1] == 0){  // right bottom
            v = gnoise21(pos);
        } else {  // right top
